...
import dev.modichamiya.eclipse.registry.*;
...
public final class EclipsePlugin extends JavaPlugin implements Listener, CommandExecutor {
    ...
    private void registerModules() {
        ...
        modules.register(new EclipseModule() {
            ...
            public void onEnable() throws Exception { Path file = getDataFolder().toPath().resolve(config.string("database.file", "eclipse.db")); provider = new SQLiteDatabaseProvider(file, config.integer("database.pool-size", 1), config.integer("database.connection-timeout-ms", 10000)); provider.start(); new MigrationRunner().migrate(provider); services.register(DatabaseProvider.class, provider); services.register(PlayerProfileService.class, new CachedProfileService(new PlayerProfileRepository(provider, databaseExecutor))); }
        ...
        modules.register(new EclipseModule() {
            public String id() { return "registry"; } public Set<String> dependencies() { return Set.of("core"); }
            public void onEnable() throws Exception { RegistryManager manager = new RegistryManager(); ContentRegistryService content = new ContentRegistryService(getDataFolder().toPath().resolve("content"), new ContentLoader(), manager, events); content.reload(); services.register(RegistryManager.class, manager); services.register(ContentRegistryService.class, content); }
            public void onDisable() { services.unregister(ContentRegistryService.class); services.unregister(RegistryManager.class); }
        });
        for (String id : List.of("config","api","assets","animation","gui","world","ai","admin","gameplay","content")) modules.register(new EclipseModule() { public String id() { return id; } public Set<String> dependencies() { return Set.of("core"); } });
    }

    @Override public void onDisable() { if (autosaveTask != null) autosaveTask.cancel(); if (modules != null) modules.disableAll(); events.clear(); services.clear(); if (databaseExecutor != null) { databaseExecutor.shutdown(); try { if (!databaseExecutor.awaitTermination(10, TimeUnit.SECONDS)) databaseExecutor.shutdownNow(); } catch (InterruptedException e) { Thread.currentThread().interrupt(); databaseExecutor.shutdownNow(); } } }
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("reload")) { try { config.load(); services.require(ContentRegistryService.class).reload(); sender.sendMessage("Eclipse configuration and content reloaded atomically."); } catch (Exception e) { sender.sendMessage("Reload rejected: " + e.getMessage()); } return true; }
        if (args.length == 2 && args[0].equalsIgnoreCase("content") && args[1].equalsIgnoreCase("reload")) { try { services.require(ContentRegistryService.class).reload(); sender.sendMessage("Content reloaded."); } catch (Exception e) { sender.sendMessage("Content reload rejected: " + e.getMessage()); } return true; }
        ...
        sender.sendMessage("Usage: /eclipse reload | content reload | module list | profile <player>"); return true;
    }

    private static final class CachedProfileService implements PlayerProfileService {
        private final PlayerProfileRepository repository; private final ConcurrentMap<UUID, PlayerProfile> cache = new ConcurrentHashMap<>(); CachedProfileService(PlayerProfileRepository repository) { this.repository = repository; }
